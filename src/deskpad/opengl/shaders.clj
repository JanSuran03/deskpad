(ns deskpad.opengl.shaders
  (:require [deskpad.util :as util]
...
(deftype ShaderProgram [^Integer shader-program-id ^:unsynchronized-mutable ^PersistentHashMap uniforms]
  ...
  (set-uniform-mat4f [this uniform-name ^"[F" mat4f]
    (let [shader-program shader-program-id]
      ...
      (if-let [uniform-location (get-uniform-location this uniform-name)]
        (GL33/glUniformMatrix4fv ^Integer uniform-location false mat4f)
...
(defn- delete-shader-program [shader-program-lookup-id]
  (GL33/glDeleteProgram (.-shader_program_id (@shader-programs shader-program-lookup-id)))
  (swap! shader-programs dissoc shader-program-lookup-id))

(defn delete-shader-programs []
  (doseq [[_ shader-program] @shader-programs]
    (GL33/glDeleteProgram (.-shader_program_id shader-program)))
  (reset! shader-programs {}))

(defn make-shader-program [shader-program-lookup-name shaders-source-path]
  (let [src-path (str shaders-root shaders-source-path)
        {:keys [vertex-source fragment-source]} (read-shaders src-path)
